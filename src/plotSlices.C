...
void plotSlices(const char* inputfilename, const char* outputFilename, int E0){
    ...
    for(Int_t i = 0; i < ana->GetEntries(); i ++){
        ...
        if(fKineticEnergy >= (Energy[0]*1000-100) && fKineticEnergy < (Energy[0]*1000+100)){slice0->Fill(fTrackLength_X0);}
        if(fKineticEnergy >= (Energy[1]*1000-100) && fKineticEnergy < (Energy[1]*1000+100)){slice1->Fill(fTrackLength_X0);}
        if(fKineticEnergy >= (Energy[2]*1000-100) && fKineticEnergy < (Energy[2]*1000+100)){slice2->Fill(fTrackLength_X0);}
    }
